...
def is_safe_youtube_url(url: str) -> bool:
    if not url or not isinstance(url, str) or len(url) > 2048:
        return False
    if any(c in url for c in FORBIDDEN_CHARS_IN_URL):
        return False
    try:
        parsed = urlparse(url)
...
class YouTubeAPI:
            ...
            if message.entities:
                ...
                for entity in message.caption_entities:
                    if entity.type == MessageEntityType.TEXT_LINK:
                        u = entity.url
                        if is_safe_youtube_url(u):
                            return u
                        return None
        if offset in (None,):
            return None
        url = text[offset : offset + length]
        return url if is_safe_youtube_url(url) else None
    ...
    async def playlist(self, link, limit, user_id, videoid: Union[bool, str] = None):
        ...
        stdout, stderr = await proc.communicate()
        if stderr:
            print("Playlist error:", stderr.decode())
